...
class ApiController
{
    ...
    public function getHouseData(string $house) {
        ...
        foreach ($allPupils[$house] as $yearGroup => $pupils) {
            ...
            foreach (collect($pupils)->sortBy('surname') as $pupil) {
                ...
                $result[ $yearGroup ][$pupil->surname.', '.$pupil->forename] = $this->getPupilTimetable(Str::before($emailAddress, "@"))['timetable'];
            }
        }
        ksort($result);
    ...
    private function sanitizeVariables($yearGroup, $setResults)
    {
        ...
        return (object) array_merge([
            'biology_set' => $setResults[ 'Biology' ],
            'chemistry_set' => $setResults[ 'Chemistry' ],
            'physics_set' => $setResults[ 'Physics' ],
        ], $output);
    }
